...
<!--
  ...
  Fill them out with the right edge cases, especially for cross-platform scenarios.
-->
...
- **FR-006**: Feature MUST work on iOS, Android, macOS, and Windows (cross-platform requirement)
- **FR-007**: Feature MUST work offline (SQLite local storage required)
- **FR-008**: UI MUST use MauiReactor MVU pattern with semantic alignment methods
- **FR-009**: All user-facing strings MUST be localized (English + Korean support)
- **FR-010**: Styling MUST use `.ThemeKey()` or MyTheme constants (no hardcoded values)
