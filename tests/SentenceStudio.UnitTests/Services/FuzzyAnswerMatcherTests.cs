...
public class FuzzyMatcherTests
{
    ...
    [InlineData("don't", "do not", true, "Contraction within Levenshtein 2 after punctuation removal")]
    [InlineData("it's", "it is", true, "Contraction within Levenshtein 2 after punctuation removal")]
    [InlineData("take picture", "take (a photo)", true, "Core word 'take' present after parenthetical stripping")]
    ...
    [InlineData("celsius degree", "Celsius", true, "Core word 'celsius' present via word-boundary match")]
    [InlineData("the take", "take (a photo)", true, "Core word 'take' present via word-boundary match")]
    [InlineData("a choose", "choose", true, "Core word 'choose' present via word-boundary match")]
    ...
    [InlineData("nite", "night", false, "Phonetic spelling with Levenshtein 3 — too distant")]
    [InlineData("lite", "light", false, "Phonetic spelling with Levenshtein 3 — too distant")]
    ...
    [InlineData("get cloud", "to get cloudy", true, "Within Levenshtein 1 after to-prefix removal")]
    [InlineData("ding sound", "ding~ (a sound)", true, "Core word 'ding' present via word-boundary match")]
    [InlineData("sound ding", "ding~ (a sound)", true, "Core word 'ding' present regardless of order")]
    [InlineData("take a photo", "take (a photo)", true, "Exact match with parentheses content")]
    [InlineData("a sound ding", "ding~ (a sound)", true, "Core word 'ding' present via word-boundary match")]
    ...
    [InlineData("celsiuss", "Celsius", true, "Levenshtein 1 — within threshold")]
    [InlineData("clsius", "Celsius", true, "Levenshtein 1 — missing 'e' within threshold")]
    [InlineData("takee", "take (a photo)", true, "Doubled letter typo")]
    [InlineData("chooose", "choose", true, "Levenshtein 1 — single extra letter")]
    [InlineData("tak", "take (a photo)", true, "Missing last letter")]
    [InlineData("choos", "choose", true, "Missing last letter")]
    [InlineData("getc loudy", "to get cloudy", true, "Levenshtein 2 after to-prefix removal")]
    [InlineData("takea photo", "take (a photo)", false, "No word match and Levenshtein too high after normalization")]
    ...
    [InlineData("remaining leftover", "remaining/leftover", true, "Word 'remaining' matches first slash alternative")]
