# Archived Decisions (Before 2026-03-18)

Archived on 2026-04-17T02:50:05Z
...
# VocabularyReview ResourceId Leak — Second Layer Fix

**Date:** 2026-03-07  
**Agent:** Wash (Backend Dev)  
**Status:** Complete — awaiting Squad review

## Problem

After shipping commit 88a0272 (Quiz/LearningResource decoupling fix) to Azure and iOS, Captain reported: **"I'm still seeing the same messages when loading the quiz"** — i.e., "All vocabulary in this resource are mastered" toast and empty-pool behavior.

The original fix addressed two generation paths in `DeterministicPlanBuilder.cs` and the route parameter construction in `PlanConverter.cs`. However, the quiz was STILL loading resource-scoped vocabulary instead of the full user pool.

## Root Cause

**Persisted plan items + direct ResourceId read bypass:**

Today's DailyPlan was likely GENERATED BEFORE the deploy and persisted to the database with `ResourceId` stamped on the VocabularyReview plan item. When Captain reopened the app after deploy, the Dashboard loaded the persisted plan item from the DB.

The problem: **Index.razor `LaunchPlanItem` method (lines 922-954) reads `item.ResourceId` DIRECTLY** and constructs the query string, bypassing the corrected `PlanConverter.BuildRouteParameters` logic entirely.

```csharp
// BEFORE (Index.razor lines 928-933) — WRONG
...
Even after our fix to the plan GENERATION, old persisted plan items still had ResourceId populated, and the LAUNCH code read it directly and passed it to the quiz.

## The Fix

### Layer 1: Guard the Launch Path (Index.razor)

Modified `LaunchPlanItem` to explicitly skip ResourceId when launching VocabularyReview activities:

**File:** `src/SentenceStudio.UI/Pages/Index.razor`  
**Lines:** 922-957 (modified lines 928-935)

```csharp
// AFTER — check activity type BEFORE reading item.ResourceId
if (item.ActivityType != PlanActivityType.VocabularyReview && !string.IsNullOrEmpty(item.ResourceId))
{
    if (multiResourceRoutes.Contains(route))
        query.Add($"resourceIds={item.ResourceId}");
    else
...
This ensures that even if a persisted plan item has ResourceId populated (from before the fix), the Dashboard will NOT pass it to the quiz when launching.

### Layer 2: Defense in Depth (VocabQuiz.razor)

Added a failsafe in the quiz loader itself: when `DueOnly=true` (SRS mode from daily plan), ignore the `ResourceIds` parameter even if an upstream caller somehow passes it.

**File:** `src/SentenceStudio.UI/Pages/VocabQuiz.razor`  
**Lines:** 629-651 (modified lines 634-637)
...
This makes the Quiz page itself robust — it will load from the full vocabulary pool when launched from a daily plan (DueOnly=true) regardless of whether upstream code mistakenly includes resourceIds in the query string.

## Why Two Layers?

1. **Index.razor fix** — prevents the leak at the source (the launch handler)
2. **VocabQuiz.razor fix** — prevents the leak at the destination (the loader)

This defense-in-depth approach ensures:
- Old persisted plan items can't cause the bug
- Future changes to the launch path won't re-introduce the bug
- The Quiz page itself "knows" to ignore ResourceIds when in SRS mode

## Build Verification

```bash
cd /Users/davidortinau/work/SentenceStudio
dotnet build src/SentenceStudio.UI/SentenceStudio.UI.csproj -c Release --no-incremental
# Result: 377 warnings (pre-existing), 0 errors ✅
```

## Impact

- **User-facing:** VocabularyReview launched from daily plan will now correctly load from full user vocabulary pool (e.g., 497 words) instead of resource-scoped subset (e.g., 20 words)
- **Backward-compatible:** Old persisted plan items from before the fix will still work correctly — the launch path now guards against them
- **Future-proof:** Quiz page itself is robust to upstream mistakes

## Files Changed

1. `src/SentenceStudio.UI/Pages/Index.razor` (lines 922-957)
   - Added `item.ActivityType != PlanActivityType.VocabularyReview` check before reading ResourceId

2. `src/SentenceStudio.UI/Pages/VocabQuiz.razor` (lines 629-651)
   - Modified LoadVocabulary to ignore ResourceIds when DueOnly=true

## Next Steps

1. Squad to review changes
2. If approved, commit with message:
   ```
   fix: VocabularyReview launch path was bypassing PlanConverter fix
   
   Index.razor LaunchPlanItem was reading item.ResourceId directly from persisted
   plan items (generated before deploy), bypassing the PlanConverter.BuildRouteParameters
   logic that removes ResourceId for VocabularyReview.
   
   Two-layer fix:
   - Index.razor: Skip ResourceId when launching VocabularyReview activity type
   - VocabQuiz.razor: Ignore ResourceIds parameter when DueOnly=true (SRS mode)
   
   This ensures VocabularyReview always loads from full user vocabulary pool,
   regardless of what's persisted on old plan items or passed by upstream callers.
   
   Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>
   ```
3. Deploy to Azure + iOS
4. Jayne to verify via e2e-testing skill
