...
- 2026-05-18: **NumberDrill Progress Footer Parity** — Replaced NumberDrill's top-of-page dot progress indicators with the Quiz-style progress footer pattern for visual + behavioral consistency across activities. **Pattern source:** `VocabQuiz.razor` lines 382–385 (`activity-footer` div with left-aligned "X / Y" progress counter + right-aligned success badge). **Implementation:** (1) Removed lines 116–123 (dot indicator loop), (2) Added footer block after `activity-content` closing tag at line 472, (3) Used EXACT same markup: `activity-footer` class, `d-flex justify-content-between align-items-center`, `ss-body1 text-secondary-ss` for progress text, Bootstrap icon `bi-check-lg` in success badge. **Key differences from Quiz:** NumberDrill uses `@(currentIndex + 1) / @session!.Items.Count` (zero-based index + 1) instead of `@(currentTurnInRound + 1) / @roundWordOrder.Count`, and hardcoded "correct" instead of localization key (localization key `NumberDrill_CorrectCount` doesn't exist yet — added to future backlog). **Gotchas:** Footer must be OUTSIDE `activity-content` closing tag (line 472) but BEFORE `activity-input-bar` (line 474) — same placement pattern as Quiz. No wrapper grid needed; `activity-footer` is its own block-level element. **Pattern established:** All input-category activity pages (VocabQuiz, NumberDrill, future Cloze/Translation) now use footer-based progress UI, not top-of-page dots. Top dot indicators reserved for multi-step wizards/onboarding flows, not session-based drills. Files: `NumberDrill.razor` (9 lines removed, 8 added). Build: ✅ clean. Commit: `77071f91`. Decision doc: `.squad/decisions/inbox/kaylee-progress-footer-parity.md`.
- 2026-05-07: **NumberDrill Progress Footer Convention** — Standardized progress UI across activity pages by replacing NumberDrill's top dot indicators with Quiz-style bottom footer ("X / Y" + success badge). Pattern sourced from VocabQuiz.razor:382–385 per Captain's UX consistency directive. Key decisions: (1) **Footer placement** — AFTER `activity-content`, BEFORE `activity-input-bar` (no grid wrapper needed). (2) **Bootstrap icon mandatory** — `bi-check-lg` for checkmark, NEVER emojis (project rule). (3) **Why not top dots?** — Dots reserved for onboarding wizards; drills with 10–20+ items overflow on mobile. Footer "7 / 20" scales better. (4) **Convention for future activities** — All new activity implementations (Cloze, Translation, Reading) use this same footer pattern + copy VocabQuiz markup exactly. Decision doc: `.squad/decisions/inbox/kaylee-progress-footer-parity.md`. Files modified: `NumberDrill.razor` (9 lines removed dots, 8 lines added footer). Commits: `77071f91` (impl) + `1b2a214c` (docs). Shipped in Publish #7 (azure api--0000093 + webapp--0000079 + iOS DX24). Build green; E2E pending Captain manual validation on DX24.

- 2026-05-18: **NumberDrill Progress Footer Layout Parity Fix** — Captain reported footer wasn't sticking to bottom edge like VocabQuiz. Root cause: `.page-header` CSS was missing `flex-shrink: 0`, allowing PageHeader to shrink in the flex container and breaking the layout calculation. Fixed by adding `flex-shrink: 0` to `.page-header` in `app.css` line 1390. **Complete flex pattern for activity pages:** (1) `activity-page-wrapper` = flex column container, (2) `page-header` = sticky + flex-shrink: 0 (don't shrink), (3) `activity-content` = flex: 1 (grow to fill), (4) `activity-footer` = flex-shrink: 0 (don't shrink) + safe-area-inset-bottom for iOS. All three flex declarations (PageHeader shrink, content grow, footer shrink) are critical — missing any one breaks bottom-pinning. Also added closing comments (`@* end activity-content *@`, `@* end activity-page-wrapper *@`) to NumberDrill for structure clarity. Files: `app.css` (1 line CSS fix), `NumberDrill.razor` (2 comments). Build: ✅ clean. Commit: `577852ff`. Updated decision doc with layout pattern reference.
