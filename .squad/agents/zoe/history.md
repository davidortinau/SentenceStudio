...
- Display Language bug (2026-04-24): Post-MauiReactor migration, Blazor UI localization is broken in three stacked ways — (1) Profile save never calls `LocalizationManager.SetCulture`; the only caller `UserProfileRepository.SaveDisplayCultureAsync` is dead code. (2) Only 1 razor file (`NavMenu.razor`) uses `@Localize[...]`; rest of UI is hardcoded English. (3) `BlazorLocalizationService` is registered `AddSingleton` — wrong for server-side Blazor (per-circuit culture leaks across users), no change-notification, no `StateHasChanged` trigger. Plus no `UseRequestLocalization` / `AddLocalization` in `WebApp/Program.cs`. Korean `.resx` is fine (1819 lines). `LocalizationManager.SetCulture` mutates process-wide `DefaultThreadCurrentUICulture` — safe in MAUI (single user) but a multi-tenant bug on server.
- Blazor localization architecture rule for this codebase: **scoped, not singleton.** Any cross-user state holder in `SentenceStudio.UI` must be `AddScoped` so the same razor library works correctly in both MAUI Hybrid (single circuit) and Blazor Server WebApp (N circuits). Plan filed at `.squad/decisions/inbox/zoe-display-language-restoration.md`.
- Phase 1 locale impl review (2026-04-24): Kaylee's implementation reviewed — ⚠️ approved with follow-ups. Architectural fit is correct: `BlazorLocalizationService` genuinely scoped, uses `LocalizationManager.GetString(key, culture)` overload so no static mutation on WebApp path, `CultureChanged` event wired with proper `IDisposable` unsubscribe in NavMenu + Profile. `/SetCulture` endpoint whitelists {en, ko} and validates `returnUrl` as relative-only — no open redirect, no header injection. MAUI `LocalizationInitializer` (`IMauiInitializeService`) applies saved profile culture before first render; blocking sync-over-async is acceptable for single-user boot. 56 en + 56 ko keys added with consistent `Nav_*` / `Profile_*` prefixes and Korean translations that are idiomatic for UI context. Build clean (0 errors). Non-blocking items filed as tech debt: culture cookie should be `HttpOnly=true` + `Secure=request-aware`; GET `/SetCulture` is CSRF-able but impact bounded to language flip; `Profile_Saved` toast on web path is destroyed by `forceLoad` reload (UX polish); orphaned legacy `Dashboard`/`Settings` resx keys for Phase 3 cleanup.
- Process note (2026-04-24): Kaylee voluntarily expanded scope from "Phase 1 plumbing only" to "Phase 1 + full NavMenu + Profile localization" under Captain's autonomous-thoroughness directive. Outcome was fine — scope was bounded, documented, and didn't leak into unrelated pages — but worth watching. When an agent self-expands scope, the review gate has to explicitly confirm the bonus work is inside the architectural envelope, not just "more" of it.
