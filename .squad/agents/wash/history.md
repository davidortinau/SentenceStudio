...
- 2026-05-06: **NumberDrill Override Button Spacing Polish Publish** — Publish #6 shipping Kaylee's CSS spacing fix (commit 33a302b8 merged as 7294a302). CHANGE: Added `mb-3` (margin-bottom-3) class to override button container div in NumberDrill.razor for improved visual spacing from Next button. CSS-only markup change, no backend logic affected. **Azure**: `azd deploy` succeeded in 2m 6s, deployed API revision `api--0000092`, webapp revision `webapp--0000078`, cache/marketing/workers to `livelyforest-b32e7d63.centralus.azurecontainerapps.io`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers scaled-to-zero, migration logs scrolled out — both non-blocking) ✅. WebApp homepage: HTTP 200 ✅. **iOS to DX24**: Built Release with net10 SDK + `-p:ValidateXcodeVersion=false` (canonical recipe, NOT global.json swap — net11p3 swap triggers 31 Razor SG errors per prior investigation), targeted production API (https://api.livelyforest-b32e7d63.centralus.azurecontainerapps.io), app bundle created successfully ✅. **FIRST INSTALL FAILED** (CoreDeviceError 4000 + NWError 57, device deep sleep killed CoreDevice tunnel). **ROOT CAUSE**: Device deep sleep state, not lock alone. Lock state insufficient to preserve control-channel tunnel. **RECOVERY**: Captain unlocked DX24 at 23:35 UTC (physical activity wake), **SECOND install attempt succeeded** ✅, installed to iPhone 15 Pro (CF4F94E3-A1C9-5617-A089-9ABB0110A09F), launched app ✅. **NWERROR 57 FINDINGS**: NWError 57 is tunnel failure during CoreDevice install handshake; deep sleep kills control channel; recovery requires unlock + tunnel re-establish (retry install). Confidence: Medium. **MANUAL VALIDATION REQUIRED**: Captain to open NumberDrill on DX24, confirm override button margin-bottom (mb-3) properly spaced from Next button.

- 2026-05-06: **NumberDrill Myriad Chunking Publish** — Fourth publish today for Kaylee's Korean number myriad chunking fix (commit 9fed5979 from squad/numbers-activity-phase-1). FEATURE: Implements full Sino-Korean myriad chunking (십만=100,000, 백만=1,000,000, 천만=10,000,000) + Native compound parsing (스물 셋=23). ROOT CAUSE: Prior parser treated ALL place markers additively (`십만` = 십+만 = 10+10,000 = 10,010) instead of multiplicative (`십만` = 십×만 = 10×10,000 = 100,000). FIX: Parser now splits tokens at myriad boundaries (만, 억), parses each chunk as 4-digit segment with place×coefficient math, multiplies chunk value by myriad scale, sums all chunks. BLOCKING ISSUE: Azure deploy initially failed with NuGet package downgrade errors — Npgsql.EntityFrameworkCore.PostgreSQL 10.0.1 requires EF Core ≥10.0.7, but Directory.Packages.props pinned 10.0.5. RESOLUTION: Bumped EF Core (10.0.5 → 10.0.7) AND all Microsoft.Extensions.* packages (10.0.5 → 10.0.7) for transitive dep alignment. **Azure**: `azd deploy` succeeded in 2m 3s on SECOND attempt after package bumps (commit c0c5b5c1), deployed API revision `api--0000090`, webapp revision `webapp--0000076`, cache/marketing/workers to `livelyforest-b32e7d63.centralus.azurecontainerapps.io`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers scaled-to-zero, migration logs scrolled out — both non-blocking). **iOS to DX24**: Switched to .NET 11 Preview 3 (global.json), built Release targeting production API (https://api.livelyforest-b32e7d63.centralus.azurecontainerapps.io) in 36.6s with 603 warnings (known package vulnerabilities), FIRST install attempt failed (connection error: Socket is not connected), retry succeeded ✅, launched app FAILED ❌ (device locked, expected behavior from Captain's warning). Restored global.json to net10. **LESSON**: Package bumps are now a recurring publish workflow step — Aspire integration packages (Npgsql, OpenTelemetry) update faster than stable EF Core. Keep a watch on `NU1605` "package downgrade" errors; they BLOCK `azd deploy` manifest generation. NEXT MANUAL VALIDATION: On DX24 after unlock, open NumberDrill Money context, type `100000원` for canonical `십만 원` → should be ACCEPTED ✅, type `1000000원` for `백만 원` → ACCEPTED ✅, type `10000000원` for `천만 원` → ACCEPTED ✅ (all within myriad chunking coverage). Decision drop: `.squad/decisions/inbox/wash-publish-4-myriad.md`.

- 2026-05-06: **NumberDrill Sino-Compound Normalizer Fix** — Full publish workflow for Kaylee's Sino-compound fixes (commit fc27ad8d from squad/numbers-activity-phase-1). BUG: User typed `1000원` for canonical `천 원` → marked wrong (system-aware grader correct, normalizer lacked Sino mapping). FIX: Kaylee added KoreanSinoCompounds dict (천=1000, 만=10000, 백=100, 십=10) in `KoreanNumberNormalizer`, handles bare-digit + counter + optional whitespace (e.g., `1000 원` OR `1000원` both → `천 원`), all 63 tests green. **Azure**: `azd deploy` succeeded in 2m 9s, deployed API revision `api--0000088`, webapp revision `webapp--0000074`, cache/marketing/workers to `livelyforest-b32e7d63.centralus.azurecontainerapps.io`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers scaled-to-zero, migration logs scrolled out — both non-blocking). **iOS to DX24**: Switched to .NET 11 Preview 3 (global.json), built Release targeting production API (https://api.livelyforest-b32e7d63.centralus.azurecontainerapps.io), initial install failed (connection error), retry succeeded ✅, launched app ✅. Restored global.json to net10. **TEST PLAN**: On DX24, type `1000원` for canonical `천 원` → should be ACCEPTED ✅, type `10000원` for `만 원` → ACCEPTED ✅, type `천원` (no space) for `천 원` → ACCEPTED ✅ (normalizer tolerates whitespace variance).

- 2026-05-06: **NumberDrill Grader System-Aware v2 Publish** — Full publish workflow for Kaylee's system-aware grading changes (commits ac88a0c8 + be1604ee from squad/numbers-activity-phase-1 @ 9e782ddb). **Azure**: `azd deploy` succeeded in 2m 8s, deployed API revision `api--0000087`, webapp revision `webapp--0000073`, cache/marketing/workers to `livelyforest-b32e7d63.centralus.azurecontainerapps.io`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers scaled-to-zero, migration logs scrolled out — both non-blocking). **iOS to DX24**: Switched to .NET 11 Preview 3 (global.json), built Release targeting production API (https://api.livelyforest-b32e7d63.centralus.azurecontainerapps.io) in 47.1s with 603 warnings (known package vulnerabilities), installed to DX24 (iPhone 15 Pro, CF4F94E3-A1C9-5617-A089-9ABB0110A09F) successfully, launched app ✅. Restored global.json to net10. **TEST PLAN**: On DX24, open NumberDrill activity with Counting/Native context (e.g., 사과 개), when prompted for "46 apples", type `46` → should be ACCEPTED ✅; type `사십육` → should be REJECTED with SinoNativeSwap error ❌. Previous behavior accepted both (wrong). Grading logic now enforces system alignment per item.System field. Backend changes: KoreanNumberNormalizer (added NumberSystem param), KoreanNumberAnswerGrader (passes item.System), KoreanNumberAnswerGraderTests (17 new test cases, 31 pass, 1 skipped). Decision drop: `.squad/decisions/inbox/wash-publish-grader-v2.md`.
...
- 2026-05-07: **Publish #7 — NumberDrill Progress Footer UX Consistency** — Full publish workflow shipping Kaylee's progress footer standardization (commits 77071f91 + 1b2a214c from main). CHANGE: Replaced NumberDrill's top dot progress indicators with bottom Quiz-style footer ("X / Y" + `bi-check-lg` badge) for UX consistency across all activity pages. Pattern sourced from VocabQuiz.razor:382–385 per Captain's directive. **Azure**: `azd deploy` succeeded in 3m 6s, deployed API revision `api--0000093`, webapp revision `webapp--0000079`, cache/marketing/workers to `livelyforest-b32e7d63.centralus.azurecontainerapps.io`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers scaled-to-zero, migration logs scrolled out — both non-blocking) ✅. WebApp homepage: HTTP 200 ✅. **iOS to DX24**: Built Release with net10 SDK + `-p:ValidateXcodeVersion=false` (canonical recipe per prior Publish #6 investigation), targeted production API endpoint ✅. **FIRST INSTALL ATTEMPT FAILED** (CoreDeviceError 4000 timeout, device deep sleep). **RECOVERY**: Captain awakened DX24, **SECOND INSTALL ATTEMPT SUCCEEDED** ✅, installed to iPhone 15 Pro (CF4F94E3-A1C9-5617-A089-9ABB0110A09F), launched app ✅. **PUBLISH STATUS**: ✅ SHIPPED (Azure + iOS). **MANUAL VALIDATION REQUIRED**: Captain to verify on DX24 — open NumberDrill, confirm dot indicators GONE from top, "X / Y" footer VISIBLE at bottom, footer updates correctly during activity. Decision doc: `.squad/decisions/inbox/wash-publish-7.md`. Bookkeeping: Merged to `.squad/decisions.md` by Scribe.


- 2026-05-07: **NumberDrill Layout Fixes Publish** — Publish #8 shipping Kaylee's two layout correctons for NumberDrill: (1) CSS fix to pin progress footer to bottom edge (flex-shrink: 0 on PageHeader), (2) remove card-ss wrapper from active session content for flat layout parity with VocabQuiz (commits 577852ff + d09c233c + 28aaca6e). **BUILD ISSUE DISCOVERED & FIXED**: Kaylee's card wrapper removal left dangling closing `</div>` (33 close vs 32 open divs). Compiler caught RZ9981 + RZ1026 unbalanced HTML tags at line 417. FIX: Removed extra `</div>`, verified balance (32 open = 32 close), retry succeeded. **Azure**: `azd deploy` succeeded in 1m 57s, deployed API revision `api--0000094`, webapp revision `webapp--0000080`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers/logs — non-blocking) ✅. **iOS to DX24**: Built Release with net10 SDK + `-p:ValidateXcodeVersion=false` (canonical recipe), targeted production API (https://api.livelyforest-b32e7d63.centralus.azurecontainerapps.io). **FIRST INSTALL FAILED** (CoreDeviceError 4000 + NWError 57, device deep sleep). **RECOVERY**: Captain unlocked DX24, **SECOND INSTALL SUCCEEDED** ✅, installed to iPhone 15 Pro (CF4F94E3-A1C9-5617-A089-9ABB0110A09F), launched app ✅. **MANUAL VALIDATION REQUIRED**: Captain to open NumberDrill on DX24, confirm: (1) footer pinned to bottom edge (not floating), (2) card wrapper removed (flat layout like VocabQuiz).

- 2026-05-07: **NumberDrill Footer Gap Removal Publish** — Publish #9 shipping Kaylee's one-line fix for footer gap (commit 0acacf5d: remove empty activity-input-bar div). ROOT CAUSE: Stray empty `<div class="activity-input-bar">` carrying border-top, padding, and safe-area-inset-bottom padding-bottom created visible strip below progress footer. VocabQuiz has no such div (hence flush footer). Code comment already flagged div as dead ("Input bar rendered above in mobile-friendly layout"). FIX: Removed dead block. **Azure**: `azd deploy` succeeded in 1m 57s, deployed API revision `api--0000095`, webapp revision `webapp--0000081`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP, 2 WARN ✅. **iOS to DX24**: Built Release with net10 SDK (0 errors), **FIRST INSTALL FAILED** (NWError 57, device deep sleep). **RECOVERY**: Captain unlocked DX24, **SECOND INSTALL SUCCEEDED** ✅, launched app ✅. **MANUAL VALIDATION REQUIRED**: Captain to confirm footer gap removed (now flush with progress bar).

- 2026-05-07: **SkillTrainer Review Applied — New Skills + DX24 Preemptive Procedure** — Five findings from publishes #5–#9 consolidated and applied: (1) DX24 NWError 57 retry pattern promoted to High confidence + preemptive wake procedure added to deploy-runbook + maui-ai-debugging skill cross-link (Wash, budget 1 retry on next publish). (2) NEW: `blazor-activity-layout-shell/SKILL.md` documenting canonical VocabQuiz shell + 4 anti-patterns (footer card wrap, empty-div chrome, unpinned footer, custom class invention). (3) NEW: `agent-progress-diagnostic/SKILL.md` with 4-step rubric for hung-agent health check before escalating. (4) CSS warning comment above `.activity-input-bar` explaining empty-div chrome trap. (5) Visual-review skill triggers broadened for implementation-vs-implementation parity (Coordinator tool now covers layout diffs). Index fix: squad-local skills now listed in `available-copilot-skills` for discoverability.

- 2026-05-08: **API Endpoint Review — Fetch-All Anti-Pattern Discovery** — Reviewed commit 398a7690 (Profile, Speech, Maintenance endpoints). THREE BLOCKING ISSUES discovered: (1) **Fetch-all anti-pattern** in ProfileEndpoints — both GET and PUT call `repository.ListAsync()` which returns ALL user profiles from database, then filters by ID in memory. Performance bomb at scale + code smell suggesting misunderstanding of repository contract. Fix: use direct DB query `db.UserProfiles.FirstOrDefaultAsync(p => p.Id == profileId)` instead. (2) **Broken multi-tenant boundary** in MaintenanceEndpoints — `/migrate-streak` extracts `user_profile_id` from claims but ignores it, calls `MigrateToStreakBasedScoringAsync()` which operates on ALL users globally. Any authenticated user can rewrite ALL users' mastery scores. Fix: add userId filter to migration OR require admin role. (3) **Zero validation** despite commit message claiming "ValidationProblemDetails on bad input" — no `ValidationProblemDetails` anywhere in diff, accepts empty strings, negative session minutes, malformed emails. Fix: add `TypedResults.ValidationProblem` for required fields + range checks. ADDITIONAL FINDINGS: Missing CancellationToken propagation (all existing endpoints use it), no logging (FeedbackEndpoints pattern), SaveAsync returns int not entity (stale data footgun), unclear error handling, hardcoded Korean default in SpeechEndpoints ignores user profile. COMMIT MESSAGE OBSERVATION: Claims validation exists when it doesn't — misleading for code archaeology. PATTERN EXTRACTED: Multi-user API endpoints must NEVER use `repository.ListAsync().FirstOrDefault(predicate)` — always scope queries by userId at DB layer for performance + correctness. Documented in `.squad/skills/api-endpoint-review-checklist/SKILL.md` with 10-point checklist covering IDOR, validation, cancellation, logging, fetch-all detection. Decision drop: `.squad/decisions/inbox/wash-fetch-all-antipattern.md`. Verdict: BLOCK — security vulnerabilities must be fixed before merge. Recommended Kaylee (profile endpoints) + Zoe (maintenance authorization model).
