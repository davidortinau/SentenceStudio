...
- 2026-05-06: **NumberDrill Grader System-Aware (Phase 1 Ships)** — Kaylee implemented system-aware grading per Captain's directive. Previous over-permissive design accepted ANY number system form regardless of counter context (pedagogically wrong). New behavior: (1) Accept bare digits universally (e.g., `46` for "마흔여섯 개"), (2) Accept linguistically-correct Korean form matching item.System, (3) REJECT wrong system (e.g., Sino for Native counter → `SinoNativeSwap` error), (4) Keep whitespace permissiveness, (5) Keep counter-mismatch detection. Files modified: KoreanNumberNormalizer (added NumberSystem param), KoreanNumberAnswerGrader (passes item.System), KoreanNumberAnswerGraderTests (17 new test cases). Test status: 31 pass, 1 skipped. Commits ac88a0c8 + be1604ee pushed to origin/squad/numbers-activity-phase-1. Grading rules are now system-aware (affects Wash/Jayne test expectations).
