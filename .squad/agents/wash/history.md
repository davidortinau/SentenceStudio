...
- 2026-05-06: **NumberDrill Override Button Spacing Polish Publish** — Publish #6 for Kaylee's NumberDrill override button spacing fix (commit 33a302b8 from squad/numbers-activity-phase-1). CHANGE: Added `mb-3` (margin-bottom-3) class to override button container div in NumberDrill.razor for improved visual spacing from Next button. CSS-only markup change, no backend logic affected. **Azure**: `azd deploy` succeeded in 2m 6s, deployed API revision `api--0000092`, webapp revision `webapp--0000078`, cache/marketing/workers to `livelyforest-b32e7d63.centralus.azurecontainerapps.io`. Post-deploy validation: 16 PASS, 0 FAIL, 2 SKIP (auth flow), 2 WARN (workers scaled-to-zero, migration logs scrolled out — both non-blocking) ✅. WebApp homepage: HTTP 200 ✅. **iOS to DX24**: Built Release with net10 SDK + `-p:ValidateXcodeVersion=false` (canonical recipe, NOT global.json swap — net11p3 swap triggers 31 Razor SG errors in ImportContent.razor per prior investigation), targeted production API (https://api.livelyforest-b32e7d63.centralus.azurecontainerapps.io), app bundle created successfully ✅. Install to DX24 (iPhone 15 Pro, CF4F94E3-A1C9-5617-A089-9ABB0110A09F) BLOCKED: CoreDeviceError 4000 + NWError 57 (Socket is not connected) — device is locked/unreachable. Per protocol: Cannot retry blindly. Awaiting Captain unlock signal, then resume install + launch via write_agent. Timestamp: 2026-05-06 23:31:32 UTC. Decision drop: `.squad/decisions/inbox/wash-publish-6-spacing.md`.
