...
public class MauiAuthenticationStateProvider : AuthenticationStateProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        {
            _logger.LogInformation("Not signed in, attempting silent refresh (3s timeout)");
            try
            {
                var signInTask = _authService.SignInAsync(); // Parameterless = silent refresh
                if (await Task.WhenAny(signInTask, Task.Delay(3000)) != signInTask)
                {
                    _logger.LogWarning("Silent sign-in timed out after 3s, proceeding as unauthenticated");
                }
                else
                {
