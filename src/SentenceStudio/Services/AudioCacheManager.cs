...
public class AudioCacheManager : IDisposable
{
    ...
    public AudioCacheManager(ElevenLabsSpeechService speechService, AudioCacheConfig? config = null)
    {
    ...
    public async Task ClearAllCacheAsync()
    {
        ...
        foreach (var filePath in filesToDelete)
        {
            try
            {
                ...
                {
                    File.Delete(filePath);
                    System.Diagnostics.Debug.WriteLine($"🏴‍☠️ AudioCacheManager: Deleted cache file {filePath}");
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"🏴‍☠️ AudioCacheManager: Failed to delete cache file {filePath}: {ex.Message}");
            }
        }
        ...
        System.Diagnostics.Debug.WriteLine($"🏴‍☠️ AudioCacheManager: Cache cleared successfully, deleted {filesToDelete.Count} files");
    }
